...
* **Stress and Confidence as 2×2 Archetype Matrices:** `/reports/stress` maps Operational Friction (Technostress) against Existential Threat (AI Job Anxiety) into four archetypes — Pressure Cooker, Obsolescence Spiral, Existential Wait, Shielded Operator (`chooseStressHook`) — each axis read against its own scale's HIGH threshold, never a shared raw-number cutoff, since Technostress is a 5-point scale and AI Job Anxiety a 7-point one. `/reports/confidence` maps Occupational Self-Efficacy against that same Total Stress reading into a second 2×2 — Unbreakable Architect, Depleted Expert, Master Operator, Competency Gap (`chooseConfidenceHook`) — framed through Conservation of Resources theory as a "Gain Spiral" vs. "Loss Spiral." Both reports personalize by demographics (Stress: Organization Type for Pressure Cooker/Existential Wait, Age Group for Obsolescence Spiral; Confidence: Experience tier for Depleted Expert/Unbreakable Architect), and Confidence's Unbreakable Architect narrative references the participant's own Grit reading directly — but only when their Grit report actually found it High, so this report can never credit a strength the Grit report didn't find. Both reports keep per-construct peer baselines (unlike Technology & Team), since here the comparison is part of the point.
