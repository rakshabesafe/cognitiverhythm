...
* **Mid-Flow Insight Hooks:** Two points in the flow pause for a brief "Analyzing…" moment before revealing a personalized, puzzling-but-informative statistic: immediately after Grit (pivoting into why the performance sections matter) and immediately after Contextual Performance (pivoting into Technostress). Each statistic is drawn from the participant's own answers plus real accumulated peer data where enough exists — never a fabricated number. The Grit hook additionally renders a full facet breakdown table (the four official Multi-Dimensional Grit Scale dimensions — Perseverance of Effort, Adaptability to Situations, Spirited Initiative, Steadfastness in Adverse Situations; Singh & Chukkali, 2021), each row showing the participant's own score against the live in-study peer average once ≥3 peers have answered that facet. No external "world average" is used for Grit — MDGS is a recently validated, India-specific instrument with no established population norms, unlike the older, structurally different Duckworth Grit-S, so citing an outside figure would misrepresent a different construct as equivalent. This sustains momentum past the point where interest most commonly drops off.
