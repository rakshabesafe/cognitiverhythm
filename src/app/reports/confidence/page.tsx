...
import { computeBenchmarkForModules, confidenceNarrative } from "@/lib/survey/scoring";
...
export default async function ConfidenceReportPage() {
  ...
  if (!responses.completedModules.includes("self-efficacy")) redirect("/dashboard");

  const allResponses = await db.listAllResponses();
  const peers = allResponses.filter((r) => r.userId !== userId);
  const narrative = confidenceNarrative(responses.answers);
  const [row] = computeBenchmarkForModules(["self-efficacy"], responses.answers, peers);
  ...
  return (
    <main className="mx-auto flex w-full max-w-lg flex-1 flex-col gap-6 px-6 py-10">
      ...
      <div>
        <p className="text-xs font-medium uppercase tracking-wide text-accent">💪 Confidence Profile</p>
